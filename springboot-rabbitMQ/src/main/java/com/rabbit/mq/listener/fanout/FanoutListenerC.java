package com.rabbit.mq.listener.fanout;
...
public class FanoutListenerC {
    private static final Logger log = LoggerFactory.getLogger(FanoutListenerC.class);

    @RabbitListener(queues = RabbitMqConfig.QUEUE_F, containerFactory = "multiListenerContainer")
    public void consumeMsgQueue(Message message, Channel channel) throws IOException {
        long tag = message.getMessageProperties().getDeliveryTag();
        try {
            log.info(Thread.currentThread().getName() + "fanoutC消费:" + new String(message.getBody()));
