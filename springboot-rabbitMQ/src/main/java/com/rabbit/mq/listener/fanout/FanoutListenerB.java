...
public class FanoutListenerB {
    private static final Logger log = LoggerFactory.getLogger(FanoutListenerB.class);

    @RabbitListener(queues = RabbitMqConfig.QUEUE_E, containerFactory = "multiListenerContainer")
    public void consumeMsgQueue(Message message, Channel channel) throws IOException {
        long tag = message.getMessageProperties().getDeliveryTag();
        try {
            log.info(Thread.currentThread().getName() + "fanoutB消费:" + new String(message.getBody()));
