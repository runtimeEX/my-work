...
public class TopicListenerB {
    private static final Logger log = LoggerFactory.getLogger(TopicListenerB.class);

    @RabbitListener(queues = RabbitMqConfig.QUEUE_C, containerFactory = "multiListenerContainer")
    // @RabbitHandler
    public void consumeMsgQueue(Message message, Channel channel) throws IOException {
        long tag = message.getMessageProperties().getDeliveryTag();
        try {
            log.info(Thread.currentThread().getName() + "B消费:" + new String(message.getBody()));
