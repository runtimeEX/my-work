...
public class FanoutListenerA {
    private static final Logger log = LoggerFactory.getLogger(FanoutListenerA.class);

    @RabbitListener(queues = RabbitMqConfig.QUEUE_D, containerFactory = "multiListenerContainer")
    public void consumeMsgQueue(Message message, Channel channel) throws IOException {
        long tag = message.getMessageProperties().getDeliveryTag();
        try {
            log.info(Thread.currentThread().getName() + "fanoutA消费:" + new String(message.getBody()));
